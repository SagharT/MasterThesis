#python3 scannumber.py 20240220/221020/MzML/*_features.csv 20240220/221020/DiaNN/*.stats.tsv
import os
import sys
import csv
import re
...
#Report = os.path.join(script_dir, 'Comparison.pdf')
Report = sys.argv[-1]
...
group2_files = [f for f in sys.argv[1:] if '.stats.tsv' in f]
...
# Optionally, call autolabel function here if you want to label the bars

plt.tight_layout()
fig.savefig(plot_filename, bbox_inches='tight')


# Process Group 2 files (TSV)
    ...
    with open(inputfile, newline='') as file:
        ...
        for row in tsv_reader:
            try:
                ...
                # Handle case where 'Precursors.Identified' is not a valid integer
                precursors_identified = 0

            # Extract ng value from filename if applicable and format it
            ng_match = ng_regex.search(inputfile)
            if ng_match:
                ng = ng_match.group(0)
                if '_' in ng:
                ...
                # Update dictionary for precursors identified counts
